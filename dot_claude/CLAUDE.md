...
**重要**: タスクに適合する定義済Skillが存在する場合、**必ずSkillを優先使用**すること。

### 実行手順（厳守）

1. **タスク受領時**: 利用可能なSkillリストを確認
2. **適合判断**: タスク内容とSkillの説明を照合
3. **明示的通知**: 「○○スキルを使用します」とユーザに伝える
4. **Skill実行**: Skillツールを呼び出す

### 禁止事項

- Skillで対応可能なのに通常ツールで代替すること
- Skillの使用をユーザに通知せずに実行すること
- 「普通のツールでもできる」という理由でSkillを回避すること
